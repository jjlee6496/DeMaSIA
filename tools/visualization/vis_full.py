...
def to_mp4(image_dir, gt, output, log_file=None):
    ...
    for image_file in image_files:
        ...
        if frame_index in gt_dict:
            ...
            for target_id in frame_gt:
                ...
                # truncation_color = (0, 255, 0) if truncation == 0 else (0, 0, 255)
                 # Map occlusion value to color
                if occlusion == 0:
                    occlusion_color = (0, 255, 0)  # Green for occlusion = 0
                elif occlusion == 1:
                    occlusion_color = (255, 0, 0)  # Red for occlusion = 1
                else:
                    occlusion_color = (0, 0, 255)  # Yellow for occlusion = 2
                # cv2.rectangle(frame, (x, y), (x + w, y + h), truncation_color, 2)
                # cv2.rectangle(frame, (x, y), (x + w, y + h), occlusion_color, 2)
                # cv2.putText(
                #     frame,
                #     f'Occ: {occlusion}',
                #     (x, y + 60),  # Adjust the position to display the text below the colored rectangle
                ...
                # # Count occurrences of occlusion values
                # if occlusion == 1:
                #     occ1_counts += 1
                # elif occlusion == 2:
                #     occ2_counts += 1
        ...
        # Log the counts of occlusion values
        if log_file:
            with open(log_file, 'a') as log:
                log.write(f'Frame: {frame_index}, Occ1 count: {occ1_counts}, Occ2 count: {occ2_counts}\n')
...
def main():
    ...
    data_dir = ['/VisDrone2019-MOT-test-dev/']
