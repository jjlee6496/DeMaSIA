...
from .visdrone_challenge_2d_box import MotChallenge2DBox
...
class VisdroneMOTChallengeDataset(CocoVideoDataset):
    ...
    CLASSES = ('ignored_regions', 'pedestrian', 'people', 'bicycle', 'car', 'van', 'truck', 'tricycle', 'awning_tricycle', 'bus', 'motor', 'others', )
    # CLASSES = ('ignored_regions', 'pedestrian', 'car', 'van', 'truck', 'bus',)
    # CLASSES = ('pedestrian', )
    def __init__(self,
    ...
    def format_track_results(self, results, infos, resfile):
        ...
        if self.interpolate_tracks_cfg is not None:
            results_per_video = interpolaprepte_tracks(
                results_per_video, **self.interpolate_tracks_cfg)

        with open(resfile, 'wt') as f:
            for frame_id, info in enumerate(infos):
                # `visdrone_frame_id` is the actually frame id used for evaluation.
                # It may not start from 0.
                if 'visdrone_frame_id' in info:
                    visdrone_frame_id = info['visdrone_frame_id']
                else:
                    visdrone_frame_id = info['frame_id'] + 1
                ...
                for i in range(len(results_per_frame)):
                    ...
                    f.writelines(
                        f'{visdrone_frame_id},{track_id},{x1:.3f},{y1:.3f},' +
    ...
    def get_dataset_cfg_for_hota(self, gt_folder, tracker_folder, seqmap):
        ...
        dataset_config = dict(
            ...
            GT_LOC_FORMAT='data/VisDrone/VisDrone2019-MOT-test-dev/annotations/{seq}.txt',
            #GT_LOC_FORMAT='{gt_folder}/{seq}/gt/gt.txt',
            #anno_cur = f'mmtracking/data/VisDrone/VisDrone2019-MOT-test-dev'
    ...
    def evaluate(self,
        ...
        if 'track' in metrics:
            ...
            for name in names:
                #gt_file = osp.join(self.img_prefix, f'/annotations/{name}.txt')
                gt_file = osp.join(f'data/VisDrone/VisDrone2019-MOT-test-dev/annotations/{name}.txt')
            ...
            dataset = [MotChallenge2DBox(dataset_config)] #trackeval.datasets.
            hota_metrics = [
